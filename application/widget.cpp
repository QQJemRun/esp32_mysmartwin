    ...
    , ui(new Ui::Widget)
{
    ...
    connect(mqtt_w,&mqtt_win::new_message,this,[=](QString msg)
        ...
        if (this->fans_state == doc.object()["items"].toObject()["fans_state"].toObject()["value"].toInt())
            ui->ch_fans_state->setEnabled(true);
        else
        {
            ui->ch_fans_state->click();
        }
...
void Widget::on_ch_fans_state_clicked()
{
    if(this->fans_state == 0)
    {
        this->fans_state = 1;
        QJsonObject message,params;
        params.insert("aqi_out_of_door", "开始通风");
        params.insert("aqi_in_the_room", "开始通风");
        params.insert("win_state",this->win_state);
        params.insert("fans_state",this->fans_state);
        params.insert("temperature",0);
        params.insert("openning_time",-1);
    ...
    else
    {
        this->fans_state = 0;
        QJsonObject message,params;
        params.insert("aqi_out_of_door", "停止通风");
        params.insert("aqi_in_the_room", "停止通风");
        params.insert("win_state",this->win_state);
        params.insert("fans_state",this->fans_state);
        params.insert("humidity",0);
        params.insert("temperature",0);
